...
RUN pacman -Syu --noconfirm yay ani-cli obs-vkcapture-git lib32-obs-vkcapture-git edex-ui-bin yt-dlp gpt4all-chat-git

# Add yay and install AUR packages
USER build
WORKDIR /home/build
RUN yay -S tochd downgrade hollywood mov-cli-git lobster --noconfirm
USER root
WORKDIR /
